...
const NearByEvents = (props) => {
    ...
    let handleMovieList = ({nearbyevents}) => {
        
        if (nearbyevents) {
            return nearbyevents.map((item) => {
                return (
                    <div >
    ...
    return (
        <div>
            <nav className="navbar navbar-inverse navbar-expand-lg navbar-light bg-light" style={{ textShadow: '2px 2px black' }}>
                ...
                <button className="btn-lg btn-dark navbar-btn m-2" onClick={HomeNavigate}><Link>Home</Link></button>
            </nav>

            <section>
                <div>
                    <h2 className='upcomingTitle'>NearBy events</h2>
